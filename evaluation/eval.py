...
def read_video_frames(path, size=(512, 512), max_frames=30):
...
def evaluate_pipeline(ref_path, target_path):
    ...
    for i in tqdm(range(n), desc="Evaluating frames"):
        f1 = ref_frames[i]
        f2 = tgt_frames[i]
...
def main():
    all_contexts = ["c2", "c3", "c4", "cp1", "t1", "t2", "l1", "l2"]
    ...
    for context in all_contexts:
        base_dir = os.path.join(os.getcwd(), "video", context)
    ...
    print("✅ Saved all results to result.csv")
